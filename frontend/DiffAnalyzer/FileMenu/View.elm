...
view model =
  let {matchingFiles, currentFile} = model
      listItems = List.map (showFile currentFile) matchingFiles
      title = el (Text Title) [ center ] <| text "Files in repository"
      columnContents = title :: fuzzyFinder :: listItems
  in column FileMenu
...
fuzzyFinder =
  ...
  in el None [ padding 10 ] <|
     Input.search TextInput [ paddingLeft 10, paddingRight 10, paddingTop 3, paddingBottom 3 ]
      searchParams
